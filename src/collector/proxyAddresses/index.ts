...
interface TokenInfo {
  ...
  astro_yearly_emmisions: number;
  distribution_schedule: Schedules;
}

export const getProxyAddressesInfo = async (): Promise<TokenInfo[]> => {
  ...
  for (const address of rewardProxyAddresses) {
    ...
    const tokenInfo: TokenInfo = {
      ...
      astro_yearly_emmisions: totalTokensPerYear * percentageDistribution,
      distribution_schedule: { values: schedule, type },
    };

    proxyAddressesInfo.push(tokenInfo);
  }
