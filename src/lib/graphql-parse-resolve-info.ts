 ...
 * requested to enable optimisations to your GraphQL schema
 * (e.g. to determine which fields are required from the backend).
 *
 * graphql-parse-resolve-info can be added as a package via
  ...
  return asts.reduce((tree, selectionVal: SelectionNode, idx) => {
    ...
    if (skipField(resolveInfo, selectionVal)) {
      ...
      if (isReserved) {
        ...
        const args: any = getArgumentValues(field, val, variableValues) || {};
